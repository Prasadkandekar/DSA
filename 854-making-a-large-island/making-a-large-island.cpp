...
public:
    int largestIsland(vector<vector<int>>& grid) {
        ...
        //step1 : connect cells containing ones
        for(int i = 0 ;i < n ;i++){
            for(int j = 0 ; j < n ;j++){
        ...
        //step 2 : try replacing 0 with ones and find larger island
        ...
        for(int i = 0 ;i < n ;i++){
            for(int j = 0 ; j < n ;j++){
                ...
                componentSize += ds.size[ds.findParent(it)];
            ...
            //last , what if there are all ones:then :
